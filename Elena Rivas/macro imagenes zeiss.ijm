...
resudir = dir + "resultados\\";
File.makeDirectory(resudir);
roidir = dir + "rois\\";
...
roiManager("Save", roidir+title+"_RoiSet.zip");
run("Bio-Formats Importer", "open=["+dir+title+"] color_mode=Default rois_import=[ROI manager] view=Hyperstack stack_order=XYCZT");
run("RGB Color");
...
saveAs("Tiff", imatiff+title+".tif");
roiManager("delete");
run("Close All");
selectWindow("Results");
saveAs("Results", resudir+title+"_Results.csv");
run("Clear Results");
